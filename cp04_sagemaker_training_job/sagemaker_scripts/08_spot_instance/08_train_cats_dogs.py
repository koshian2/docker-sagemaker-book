...
class EfficientNetB0(pl.LightningModule):
    def __init__(self):
        super().__init__()
        self.model = timm.create_model("efficientnet_b0", num_classes=2, pretrained=True)
...
class CatsAndDogsModule(pl.LightningDataModule):
    def __init__(self, opt):
        super().__init__()
        all_data = torchvision.datasets.DatasetFolder.make_dataset(
            opt.data_dir,  {"Cat":0, "Dog":1}, is_valid_file=self.is_valid_file)
    ...
    def prepare_data(self):
        self.trainset = CatsAndDogsDataset(
            self.train_data, transform=transforms.Compose([
                transforms.RandomResizedCrop(224),
                transforms.RandomHorizontalFlip(),
                transforms.ToTensor()       
        ]))
        self.valset = CatsAndDogsDataset(
...
def find_latest_checkpoints(opt):
    ckpts = sorted(glob.glob(opt.ckpt_dir+"/*.ckpt"))
    if len(ckpts) == 0:
        return None
    else:
        return ckpts[-1]
...
def main(opt):
    ...
    resume_ckpt = find_latest_checkpoints(opt)

    trainer = pl.Trainer(max_epochs=20, callbacks=[ckpt], accelerator="gpu", devices=1, enable_progress_bar=False)
    trainer.fit(model, dataset, ckpt_path=resume_ckpt) 

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Cats and dogs training")
    parser.add_argument("--data_dir", type=str, default="./data/PetImages")
